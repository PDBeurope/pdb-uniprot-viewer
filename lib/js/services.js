(function () {
  ...
  angular.module('pdb.uniprot.viewer.services', ['pdb.common.services'])
	.service('uniprotViewerService', ['$http', '$q', '$filter', 'commonServices', function($http, $q, $filter, commonServices){
		
		this.getUnipdbData = function(uniprotId){
			
			var _this = this;
		  
		  var deferred = $q.defer();
		  
		  //Get Mappings and best Mappings data
			var apiNameList = ['mappings','bestStructures','pdbUniprotDetails','uniProtToPfam'];
			var promiseList = commonServices.createPromise([uniprotId], apiNameList);
			
			commonServices.combinedDataGrabber(promiseList, [uniprotId], apiNameList, false).then(function(result) {
				
				if(result[0] == null || result[2] == null){
					if (window.console) console.log('Error : Uniprot API service request failed!');
					deferred.reject('fail response');
				}

				var bestStrMappingData = undefined;
				if(result[1] != null){
					bestStrMappingData = result[1].data[uniprotId];
				}
				
				var pfamApiData = undefined;
				if(result[3] != null){
					pfamApiData = result[3];
				}

				//parse uniprot data
				var parsedUniprotData = result[2].data;
				
				var combinedApiData = {
					uniprotId: uniprotId,
					unipdbHeading: parsedUniprotData.unipdbHeading,
					uniprotSeq: parsedUniprotData.uniprotSeq,
					pdbMappings: result[0].data[uniprotId]['PDB'],
					bestStrMappings: bestStrMappingData,
					pfamApiData: pfamApiData
				};

				deferred.resolve(combinedApiData);  
